...
describe('createConstraint', function() {
  ...
  it('should check required spec', function() {
    var constraint = createConstraint({
      name: 'name',
      validator: function() {},
      spec: {
    ...
    expect(function() {
      constraint({});
    }).toThrow('Constraint "name" spec property "a" is invalid: Value should not be empty');

    expect(function() {
      constraint({a: 'a'});
    }).toThrow('Constraint "name" spec property "a" is invalid: Value should be a number');
